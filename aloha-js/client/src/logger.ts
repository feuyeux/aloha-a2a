...
/**
 * Initialize file logging for the JS/TS client.
 * Call once at startup with the transport mode.
 */
export function initLogFile(transport: string): void {
    try {
        fs.mkdirSync(LOG_DIR, { recursive: true });
        const logPath = path.join(LOG_DIR, `js-client-${transport}.log`);
        logStream = fs.createWriteStream(logPath, { flags: 'a' });
        const ts = formatTimestamp();
        logStream.write(`${ts} - client.main - INFO - Log file: ${logPath}\n`);
    } catch (err) {
