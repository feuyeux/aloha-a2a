...
class DiceAgent:
    ...
    async def _start_jsonrpc(self, request_handler):
        ...
        try:
            ...
            logger.info("============================================================")
            logger.info("Dice Agent is running with the following transports:")
            logger.info(f"  - JSON-RPC:     http://{self.host}:{self.jsonrpc_port}")
            logger.info(f"  - Agent Card:   http://{self.host}:{self.jsonrpc_port}/.well-known/agent-card.json")
            logger.info("============================================================")
            
            await server.serve()
        except Exception as e:
            logger.error(f"Failed to setup JSON-RPC transport on port {self.jsonrpc_port}: {e}", exc_info=True)
            raise
    ...
    async def _start_grpc(self, request_handler):
        ...
        try:
            ...
            logger.info("============================================================")
            logger.info("Dice Agent is running with the following transports:")
            logger.info(f"  - gRPC:         {self.host}:{self.grpc_port}")
            logger.info(f"  - Agent Card:   http://{self.host}:{self.rest_port}/.well-known/agent-card.json")
            logger.info("============================================================")
            ...
            logger.error(f"Failed to setup gRPC transport on port {self.grpc_port}: {e}", exc_info=True)
            raise
