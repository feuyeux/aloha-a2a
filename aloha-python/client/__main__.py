...
@click.command()
@click.option("--host", default="localhost", help="Agent hostname (default: localhost)")
@click.option(
    "--port",
    type=int,
    default=None,
...
@click.option("--probe", is_flag=True, help="Probe transport capabilities and exit")
def main(host: str, port: int, transport: str, message: str, probe: bool):
    ...
    # Send message using REST (default)
      python -m client --message "Roll a 20-sided dice"

      # Send message using JSON-RPC
      python -m client --transport jsonrpc --port 13001 --message "Is 17 prime?"

      # Send message using REST
      python -m client --transport rest --port 13002 --message "Check if 2, 7, 11 are prime"
    """
